...
if(isset($_GET['search'])){ // page set hase tao id batvse other wise nahi batve
  $search_term = mysqli_real_escape_string($conn,$_GET['search']);// url ma lidhi 
}
      ...
      <div class="col-md-8">
        ...
        <div class="post-container">
          ...
          <?php
              ...
              WHERE p.title LIKE '%{$search_term}%' OR p.description LIKE '%{$search_term}%' OR p.category LIKE '%{$search_term}%'  -- p.category thi je id select hase teni j post avse jemke sport select hase tao sport ni news avse
          ...
          <?php
            ...
            if (mysqli_num_rows($result1)) {
              ...
              if ($page > 1) {
                echo '<li><a href = "author.php?search=' . $search_term . '&page=' . ($page - 1) . '">Prev</a></li>';  // catid and page ni id api 6 2 id sath url apva mate bane ni center ma & no use thy 
              }
              ...
              for ($i = 1; $i <= $total_page; $i++) {
                ...
                echo '<li class = "' . $active . '"><a href = "author.php?search=' . $search_term . '&page=' . $i . '">' . $i . '</a></li>';   // url ma superglobal variable get thi page number lese
              }
              if ($total_page > $page) {
